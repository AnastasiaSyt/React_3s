...
describe('Testing components Card', () => {
  afterAll(cleanup);
  beforeEach(() => {
    render(<Modal isOpen={isModalOpen} onClose={handleCloseModal} cardData={testCard} />);
  });

  test('the modal has image', () => {
    const image = screen.getByRole('modal_image');
    expect(image).toBeInTheDocument();
    expect(image).toHaveClass('modal_image');
  });

  test('the modal has title', () => {
    const title = screen.getByRole('modal_title');
    expect(title).toBeInTheDocument();
    expect(title).toHaveClass('modal_title');
  });

  test('the modal has author', () => {
    const author = screen.getByRole('modal_author');
    expect(author).toBeInTheDocument();
    expect(author).toHaveClass('modal_author');
  });

  test('the modal has date', () => {
    const date = screen.getByRole('modal_date');
    expect(date).toBeInTheDocument();
    expect(date).toHaveClass('modal_date');
  });
});
